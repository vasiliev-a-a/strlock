...
func TestManager_AcquireLockNilContext(t *testing.T) {
	...
	release, err := m.AcquireLock(nil, testKey)
	if err == nil {
		t.Fatalf("expected an error")
	}
	var e Error
	if !errors.As(err, &e) {
		t.Fatalf("expected `strlock.Error`, got %T", err)
	}
	if !errors.Is(err, ErrNilContext) {
		t.Fatalf("expected %q error, got %q", ErrNilContext, err)
	}
	if release == nil {
		t.Fatalf("expected a non-nil no-op release func on error")
	}
...
func TestManager_CancelledContext(t *testing.T) {
	...
	_, err := m.AcquireLock(ctx, testKey)
	if err == nil {
