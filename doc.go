...
//	defer release() // safe even if err != nil
//	if err != nil {
//		// handle error
//	}
//
...
//
// If AcquireLock returns an error, the returned function is a non-nil no-op:
// calling it has no effect. This makes the `defer release()` pattern safe
// unconditionally, without a nil check or an early return on error.
//
// # Context cancellation
//
// [Manager.AcquireLock] respects the provided context:
//
...
// # Error handling
//
// Every error returned by this package has type [Error]. Callers can use
// [errors.As] to check whether an error originated from strlock:
//
//	var e strlock.Error
//	if errors.As(err, &e) {
//		// the error came from strlock
//	}
//
// Specific error conditions should be inspected with [errors.Is] rather than
// string matching:
//
...
//
//   - [context.Canceled] and [context.DeadlineExceeded], wrapped in an [Error],
//     are returned when the context is cancelled before the lock is acquired.
//     The wrapped cause remains reachable through [Error.Unwrap].
//
// For example:
//
